import React from 'react'
import { Link } from 'react-router-dom';
import { useSelector } from 'react-redux'
import { selectUser } from '../App/slice'
import { signOut } from 'firebase/auth';
import { auth } from '../firebase'


const Profile = () => {
    const user = useSelector(selectUser);

    const logout = () => {
        signOut(auth)
    }

  return (
    <div className='bg-neutral-950 h-screen'>   
        <nav className=" flex h-10 px-4 w-full md:h-16 md:px-14 items-center justify-between" >

        <Link to={'/'}><img src="https://upload.wikimedia.org/wikipedia/commons/7/7a/Logonetflix.png" alt="" className=" h-4 mr-5 md:h-8 md:w-28 cursor-pointer" /></Link>

            <img
                src="data:image/png;base64,"
                alt="Netflix avatar Logo"
                className="h-6 md:h-7 rounded-sm"
            />
        </nav>
        <div className='mt-20 mx-auto w-fit'>
            <h1 className='text-6xl'>Edit Profile</h1>
            <div className='mt-5 flex gap-7'>
                <img src="data:image/png;base64," 
               className='h-16'
               alt="" />
                <div>
                    <h2 className='bg-gray-800 text-sm py-1 px-10 rounded-sm'>{user.email}</h2>
                    <div className='mt-5'>
                        <h3 className='mt-2'>Plans</h3>
                        <button className='mt-2 bg-netflixRed py-2 px-10 w-60 rounded-sm' onClick={logout}>Sign Out</button>
                    </div>
                </div>
            </div>
        </div>
    </div>
  )
}

export default Profile
